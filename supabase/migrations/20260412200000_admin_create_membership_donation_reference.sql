-- Optional explicit donation portion + reference on initial payment when admin creates membership.

drop function if exists public.admin_create_membership(uuid, smallint, text, text, numeric, text, date, text);
...
create or replace function public.admin_create_membership(
  ...
  p_payment_date date,
  p_notes text,
  p_donation_portion numeric default null,
...
begin
  ...
  if p_donation_portion is null then
    ...
    v_donation_amount := round(
      least(greatest(coalesce(p_donation_portion, 0), 0), p_amount::numeric)::numeric,
      2
    );
    v_membership_amount := round((p_amount::numeric - v_donation_amount)::numeric, 2);
    if v_membership_amount < 0 then
      return jsonb_build_object('ok', false, 'error', 'invalid_split');
    end if;
    if v_fee is not null and v_membership_amount > round(v_fee::numeric, 2) + 0.001 then
      return jsonb_build_object('ok', false, 'error', 'membership_overpay');
    end if;
  end if;

  if round((v_membership_amount + v_donation_amount)::numeric, 2) <> round(p_amount::numeric, 2) then
    return jsonb_build_object('ok', false, 'error', 'invalid_split');
  end if;
...
revoke all on function public.admin_create_membership(uuid, smallint, text, text, numeric, text, date, text, numeric, text) from public;
revoke execute on function public.admin_create_membership(uuid, smallint, text, text, numeric, text, date, text, numeric, text) from anon, authenticated;
grant execute on function public.admin_create_membership(uuid, smallint, text, text, numeric, text, date, text, numeric, text) to service_role;

comment on function public.admin_create_membership(uuid, smallint, text, text, numeric, text, date, text, numeric, text) is
  'Create membership (pending or with manual payment + status sync). Optional donation portion and payment reference. service_role only.';
