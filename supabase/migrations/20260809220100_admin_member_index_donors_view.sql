...
begin
  if v_view not in ('voting', 'associate', 'mailing', 'pending', 'lapsed', 'incomplete', 'all', 'duplicate', 'donors') then
  ...
  associate_f as (
    ...
    where m.status::text <> 'disabled'
      and exists (
        select 1 from public.memberships ms
        where ms.member_id = m.id and ms.year = v_year
          and ms.status = 'active' and ms.tier = 'associate'
      )
  ),
  mailing_f as (
    select m.id from public.members m
  ...
  duplicate_keys as (
    select public.normalize_lake_address_key(m.lake_civic_number, m.lake_street_name) as key
    from public.members m
    where (v_inc_dis or m.status::text <> 'disabled')
      and public.normalize_lake_address_key(m.lake_civic_number, m.lake_street_name) is not null
    group by key
    having count(*) > 1
  ),
  duplicate_f as (
    select m.id from public.members m
    where (v_inc_dis or m.status::text <> 'disabled')
      and public.normalize_lake_address_key(m.lake_civic_number, m.lake_street_name) in (select key from duplicate_keys)
  ),
  donors_f as (
    select m.id from public.members m
    where m.status::text <> 'disabled'
      and exists (
        select 1 from public.memberships ms
        join public.payments p on p.membership_id = ms.id
        where ms.member_id = m.id and ms.year = v_year
          and p.donation_amount > 0
      )
  )
  select jsonb_build_object(
    'voting', (select count(*)::int from voting_f),
    'associate', (select count(*)::int from associate_f),
    ...
    'duplicate', (select count(*)::int from duplicate_f),
    'donors', (select count(*)::int from donors_f)
  )
  into v_counts;
      ...
      and (
        ...
        or (
          case v_view
            ...
            when 'duplicate' then
              (v_inc_dis or e.status::text <> 'disabled')
              and public.normalize_lake_address_key(e.lake_civic_number, e.lake_street_name) is not null
              and exists (
                select 1 from public.members m2
                where m2.id <> e.id
                  and (v_inc_dis or m2.status::text <> 'disabled')
                  and public.normalize_lake_address_key(m2.lake_civic_number, m2.lake_street_name)
                    = public.normalize_lake_address_key(e.lake_civic_number, e.lake_street_name)
              )
            when 'donors' then
              e.status::text <> 'disabled'
              and exists (
...
revoke all on function public.admin_member_index(p_view text, p_year smallint, p_lapsed_since smallint, p_include_disabled boolean, p_q text, p_sort text, p_limit integer, p_offset integer) from public;
grant execute on function public.admin_member_index(p_view text, p_year smallint, p_lapsed_since smallint, p_include_disabled boolean, p_q text, p_sort text, p_limit integer, p_offset integer) to service_role;
comment on function public.admin_member_index(p_view text, p_year smallint, p_lapsed_since smallint, p_include_disabled boolean, p_q text, p_sort text, p_limit integer, p_offset integer) is
