...
begin
  if v_view not in ('voting', 'associate', 'mailing', 'pending', 'lapsed', 'incomplete', 'all', 'duplicate', 'uncategorized_donors') then
    ...
    where m.status::text <> 'disabled'
      and exists (
        ...
        where ms.member_id = m.id
          and extract(year from p.date) = v_year
          and p.donation_amount > 0
          and p.donation_category is null
      )
  )
  select jsonb_build_object(
    ...
    'uncategorized_donors', (select count(*)::int from uncategorized_donors_f)
  )
  into v_counts;
      ...
      and (
        ...
        or (
          case v_view
            ...
            when 'uncategorized_donors' then
              e.status::text <> 'disabled'
              and exists (
                select 1 from public.payments p
                join public.memberships ms on ms.id = p.membership_id
