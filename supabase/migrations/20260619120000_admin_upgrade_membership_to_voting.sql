-- Admin: upgrade associate membership to voting in-place (same year). Voting eligibility enforced; sync sets pending until dues meet voting fee.
...
comment on function public.admin_upgrade_membership_to_voting(uuid) is
  'Set membership tier from associate to voting; re-sync status from payments. service_role only.';

-- Stripe: cumulative dues cap (parity with record_manual_payment)
