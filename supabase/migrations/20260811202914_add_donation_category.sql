drop function if exists "public"."record_manual_payment"(p_membership_id uuid, p_amount numeric, p_membership_amount numeric, p_donation_amount numeric, p_method text, p_payment_date date, p_notes text, p_donation_note text, p_reference text);

drop function if exists "public"."record_stripe_payment"(p_membership_id uuid, p_amount numeric, p_membership_amount numeric, p_donation_amount numeric, p_stripe_payment_id text, p_notes text, p_donation_note text, p_stripe_fee_cad numeric, p_stripe_balance_transaction_id text);

alter table "public"."payments" add column "donation_category" text;

alter table "public"."payments" add constraint "payments_donation_category_check" CHECK (((donation_category IS NULL) OR (donation_category = ANY (ARRAY['environment'::text, 'regatta'::text, 'general'::text])))) not valid;

alter table "public"."payments" validate constraint "payments_donation_category_check";
...
revoke all on function public.record_stripe_payment(p_membership_id uuid, p_amount numeric, p_membership_amount numeric, p_donation_amount numeric, p_stripe_payment_id text, p_notes text, p_donation_note text, p_stripe_fee_cad numeric, p_stripe_balance_transaction_id text, p_donation_category text) from public;
grant execute on function public.record_stripe_payment(p_membership_id uuid, p_amount numeric, p_membership_amount numeric, p_donation_amount numeric, p_stripe_payment_id text, p_notes text, p_donation_note text, p_stripe_fee_cad numeric, p_stripe_balance_transaction_id text, p_donation_category text) to service_role;
comment on function public.record_stripe_payment(p_membership_id uuid, p_amount numeric, p_membership_amount numeric, p_donation_amount numeric, p_stripe_payment_id text, p_notes text, p_donation_note text, p_stripe_fee_cad numeric, p_stripe_balance_transaction_id text, p_donation_category text) is
