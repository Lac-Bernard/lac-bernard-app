...
export const POST: APIRoute = async ({ request, cookies, params }) => {
	...
	const { data: rpcResult, error: rpcError } = await service.rpc('admin_remove_membership_complimentary', {
	...
	if (!result?.ok) {
		...
		const status = code === 'not_found' ? 404 : code === 'not_complimentary' ? 409 : 400;
	...
	await insertAdminAudit(service, {
		...
		action: 'membership_admin_remove_complimentary',
